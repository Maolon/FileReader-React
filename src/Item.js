...
const testItems = {
  "0": {
    ...
    name: "square.cc",
    referer:
      "https://www.eecs.yorku.ca/course_archive/1995-96/cpp/cppPuzzles/square.cc",
    season: null,
    status: 3,
    type: "cc",
    url:
...
class Item extends React.Component {
  ...
  getFiles = () => {
    ...
    t0 = performance.now();
    fetch("https://files.mai1015.com/api/item?c=50000", options)
      .then(resp => resp)
      .then(data => data.json())
      .then(data => {
        t1 = performance.now();
  ...
  organizeData = data => {
    ...
    for (var i = 0; i < curData.length; i++) {
      ...
      if (code != -1) {
        var year = this.isItemOf(
          newData[code].children,
          "name",
          curData[i].year
        );
        if (year != -1) {
          var season = this.isItemOf(
            newData[code].children[year].children,
            "name",
            curData[i].season
          );
          if (season != -1) {
          ...
          return (
            <div style={styles.fadeIn}>
              {this.state.items.map(items => (
                <Item_directory item={items} render={true} key={items.name} />
              ))}
            </div>
          );
        } else {
          return <div style={styles.fadeIn}>{this.handleSearch()}</div>;
        }
      } else {
        return (
          <div>
